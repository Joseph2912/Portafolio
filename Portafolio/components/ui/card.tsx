...
function Card({ className, size = "default", ...props }: React.ComponentProps<"div"> & { size?: "default" | "sm" }) {
    return (
        <div
            ...
            data-size={size}
            className={cn(
                "group/card flex flex-col gap-4 overflow-hidden rounded-[var(--radius-surface)] border border-border/70 bg-card py-4 text-sm text-card-foreground has-data-[slot=card-footer]:gap-0 has-data-[slot=card-footer]:pb-0 has-data-[size=sm]:gap-3 has-data-[size=sm]:py-3 has-data-[size=sm]:has-data-[slot=card-footer]:gap-0 has-data-[size=sm]:has-data-[slot=card-footer]:pb-0",
                className,
            )}
            {...props}
        />
